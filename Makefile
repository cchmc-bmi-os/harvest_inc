...
setup:
	@if [ ! -f ./harvest_inc/conf/local_settings.py ] && [ -f ./harvest_inc/conf/local_settings.py.sample ]; then \
	    echo 'Creating local_settings.py...'; \
	    cp ./harvest_inc/conf/local_settings.py.sample ./harvest_inc/conf/local_settings.py; \
	fi;
